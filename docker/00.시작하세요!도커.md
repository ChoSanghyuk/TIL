...
- 옵션
  ...
  - :bulb: `--user "$(id -u)":"$(id -g)"`
    ...
    - sets the user and group ID for the process running inside the container
    - `$(id -u)` : Gets your current user ID (UID) on the host system
    - `$(id -g)` : Gets your current group ID (GID) on the host system
...
- 빌드 : `docker build [-t {이미지 이름}] [-f {Dockerfile 경로}] {빌드컨텍스트}/`

  - ex) `docker build -t proxy_img -f server_proxy/Dockerfile .`
  - `-t` : 생성될 이미지의 이름을 설정
  - `-f` : Dockerfile이 위치한 경로. 기본값은 `.`
  - 빌드 컨텍스트
...
```bash
# Backup existing MySQL container data
...
# Start new MySQL container with volume
docker run -d --name mysql-container \
  -e MYSQL_ROOT_PASSWORD=yourpassword \
  -v /host/mysql_data:/var/lib/mysql \
  -p 3306:3306 \
  mysql:latest


# Restore data into new container
