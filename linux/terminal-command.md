...
- zip
  ...
  - :bulb: 특정 폴더 하위를 전체 제외하되, 그 중 하나만은 포함하기
  
    ```bash
    zip {파일명}.zip {대상폴더}/* -x "{대상폴더}/{제외폴더}/*"
    zip -r {파일명}.zip {대상폴더}/{제외폴더}/{제외제외폴더} # 기존 압축 폴더에 포함시켜줌
    ```
...
- 전체 이력 삭제
  ...
  - zsh
    ...
    ```sh
    cat /dev/null > ~/.zsh_history && source ~/.zsh_history
    ```

  :bulb: 전체 삭제 후 현재 terminal까지 닫고 새 terminal을 열어야 이전 이력들 전체 안 보임

- 이력 기능 비활성화

  - `~/.bashrc` 혹은 `~/.zshrc` 파일에 `unset HISTFILE` 라인 추가
