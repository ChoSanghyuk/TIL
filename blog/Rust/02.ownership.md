...
It's worth noting that Rust uses a "**RAII**" (Resource Acquisition Is Initialization) pattern, where resources like memory are automatically deallocated when their owning value goes out of scope. This helps prevent common memory-related bugs like use-after-free and memory leaks.
...
한가지 value에 대해 한 변수만 접근 할 수 있는 것은 X
