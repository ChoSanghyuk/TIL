...
- interfaces should typically be defined by the consumer, not the provider
  - opposite to languages like Java/C#.
- interfaces 정의 위치
  - [best] in the {counsumer}.go
  - [recommended] in the {consumer}_interfaces.go
  - [bad] interfaces.go (too abstract)
- import 충돌 시,
  - custom package를 alias. (well-known standard library는 가급적 그대로 둠)
...
- `io.Writer` interface에 작성하며, `os.Stderr` 을 사용함
...
| Log info/debug                      | ✅                | ✅                         |
| Log warnings/errors                 | 🚫                | ✅                         |
| Output to a file: `./app > out.txt` | ✅ (goes to file) | ❌ (still shows on screen) |
