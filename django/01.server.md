...
- app 폴더 생성
- project 이름 뒤에 . 붙이면 현재 폴더에서 프로젝트 실행
...
​	URl에서 값을 원하는 타입으로 변경하는 기능으로, '.../\<path converter : value>/..' 형식으로 작성한다.

- str : 디폴트 값으로 문자열을 반환
- int : 0 이상의 정수
- slug : 영문 대소문자와 숫자, 그리고 하이폰(-)과 밑줄(_)을 갖는 문자열
- path : 경로에 사용되는 슬래쉬(/)를 포함하는 문자열
