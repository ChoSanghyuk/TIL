...
- 양자화
  - 모델의 숫자를 **더 작은 단위로 압축**하는 기술로, LLM의 파라미터의 기존 **BF16(16비트)** 형식을 **8비트(FP8)**, **4비트(INT4)** 등으로 줄이는 기술. 
  - 모델의 용량이 감소하지만, 성능 저하를 동반. FP8 양자화까지는 성능 저하가 미비한 것으로 확인.
...
- 역할
  ...
  - 하드웨어와 응용 프로그램 사이에서 인터페이스를 제공
  - 컴퓨터의 물리적(하드웨어) 자원과 추상화 자원을 관리

- 특징

  - 커널은 항상 컴퓨터 자원들을 바라만 보고 있기에 사용자와의 상호작용은 지원하지 않음
...
- **thread block** 
  - a 1D, 2D, or 3D group of threads that cooperate and share data
  - The Logic Level
  - All threads within the same block can "talk" to each other via shared memory and can be synchronized 
- **lane (or thread)** 
  - the smallest unit of execution within a warp (commonly 32 or 64 threads) that executes the same instruction simultaneously (SIMT/SIMD) just on different data
  - The Hardware Level
...
- RPO (Recovery Point Objective, 목표 복구 시점))
  - 재해나 장애 발생 시 비즈니스 연속성을 위해 허용할 수 있는 최대 데이터 손실 시간
  - 마지막 백업 시점부터 장애 발생 시점까지의 간격으로, RPO가 짧을수록 데이터 손실이 최소화

- CDC (change data capture, 변경 데이터 캡처)
  - 데이터 변경 사항을 추적하고 이러한 변경에 대응해야 하는 다른 시스템 및 서비스에 알림을 전송하는 데이터 통합 패턴
...
- **Swap Space**
  - = DB 스왑, Swap File
  - RAM이 꽉 찼을 때 HDD/SSD의 일부를 RAM처럼 사용하여 메모리 부족을 해결하는 가상 메모리 기술
- 
...
- **RSA** (**Rivest-Shamir-Adleman**)
  - 인수분해 문제 해결의 높은 난이도를 이용한 가장 대표적인 공개키 암호 알고리즘

  - 암호화뿐만 아니라 전자서명의 용도로도 사용

- **OAEP** (**Optimal Asymmetric Encryption Padding**)
  - RSA를 개량한 알고리즘
  - 평문 해시값과 정해진 개수의 '0' 등으로 만들어진 '인증 정보'를 평문의 앞에 추가하고 RSA로 암호화
...
- reentrancy attack
  ...
  - 재진입성(Reentrancy)은 함수가 실행을 완료하기 전에 중단되었다가 다시 호출되어도 안전하게 작동하는 성질
  
  - 스마트 컨트랙트(Solidity) 환경에서는 외부 컨트랙트 호출을 악용하여 상태 변경 전 함수를 반복 호출하여 자금을 탈취하는 주요 보안 취약점
  
  - EVM은 single-thread이고, `call` 함수를 사용할 시 현재 동작을 멈추고 receiving contract에게 실행 주도권이 넘어가는 특성을 활용. 
  
    ```solidity
    // BAD: The "Check-Interactions-Effects" mistake
    function withdraw() public {
        uint bal = balances[msg.sender];
        require(bal > 0);
    
        // 1. INTERACTION: Sending money triggers the attacker's code
        (bool success, ) = msg.sender.call{value: bal}("");
    
        // 2. EFFECT: This line is NEVER reached until the attacker stops calling withdraw
  ...
  - 방어
    ...
    - **CEI(Checks-Effects-Interactions) 패턴:** 검사 -> 상태 업데이트(Effect) -> 상호작용(Call) 순서로 코드 작성.
  
      **Mutex Lock:** 함수 실행 중에 다시 진입하지 못하도록 잠금 장치 설정
...
- Bin steps
  ...
  - 개요
    - **discrete liquidity buckets** used by **Trader Joe v2 (Liquidity Book)** and some other “bin-based” AMMs.
...
- ticks
  ...
  - 개요
    - a **specific price** on a DEX that uses **concentrated liquidity** (Uniswap v3, Algebra CL, ..)
  - bins 비교
    - bin : 하나의 bin에 유동성이 담겨있음
    - tick : Liquidity is provided **between two ticks** → a price range.

- sqrtPrice

  - AMM에서 사용하는 가격 지표

  - While **ticks** are discrete, fixed price points, the **actual pool price** (`sqrtPrice`) is a **continuous** value
    - :bulb: SqrtPrice는 두 tick 사이의 값이기에, safelyGetStateOfAMM 결과로 나오는 sprtPrice 랑 tick은 완벽하게 일치하진 않음

- TVL

  - Total Value Locked

  - 프로토콜에 스테이킹되어 있거나 예치되어 있는 자산의 합계
  
- Concentrated Liquidity Market Maker (CLMM)
  - LB(Liquidity Book) divides the price space into discrete, non-overlapping segments called **"bins."**
  - **Liquidity Providers (LPs)** place their capital into these specific bins
  - CL{num}
    - 유동성을 공급할 때, lower bound와 upper bound는 반드시 {num}의 배수여야 한다.
    - 예시, CL200 => {"tickLower": -249600, "tickUpper": -248400}
  - CL 종류
    - CL200 : 200 bps (Volatile / Exotic Pairs)
    - CL50 : 50 bps (Major Pairs)
    - CL1 : 1 bps (Stablecoin Pairs)

- Virtual Automated Market Maker(vAMM)

  - the "liquidity" is virtual. It uses the constant product formula `x * y = k`, but x and y are just numbers in a smart contract

  - 특징
    - **No Liquidity Providers (LPs) needed:** In its original form (v1), there are no LPs to provide assets. The "pool" is just a price discovery tool.
    - **The Vault:** Instead of a pool, there is a **Vault** where all traders deposit collateral (usually USDC). When you "buy," the vAMM calculates how much the price should move, and your profit or loss is later settled from the Vault.
    - **No Impermanent Loss:** Since there are no real assets in the pool being rebalanced against the market, LPs (if they exist in later versions) don't suffer the classic impermanent loss from price divergence.
  - 단점
    - The "Pure vAMM" (pioneered by Perpetual Protocol v1) struggled because it couldn't handle massive volume without the price swinging wildly (high slippage).

  - https://gemini.google.com/app/3b1fe4d5c2050457?hl=ko
- APR
- Annual Percentage Rate
  - the amount of interest or rewards you earn on your assets over the course of a year
...
- 상태
  ...
  - pending
    ...
    - It has been broadcast to the network but hasn't been picked up by a miner or validator yet.

  - mined

    - A miner or validator selects your transaction from the mempool and includes it in a new block.
    - Once that block is added to the blockchain, the transaction is "mined."

  - reOrged (Chain Reorganization)

    - A **Re-Org** occurs when two miners produce blocks at nearly the same time, creating two competing versions of the chain.

    :bulb: POS에서 한 slot당 한 명의 validator가 배정되기에 보통 발생하진 않으나, Malicious 공격 혹은 Network Latency로 인해 발생할 수도 있음.

  - confirmed

    - As more blocks are added on top of the block containing your transaction, the number of **confirmations** increases

  - finalized

    - Finality is the point at which the transaction is legally and technically impossible to revert or alter.
...
- private
  - QBFT
    - Byzantine Fault Tolerant (BFT) Proof of Authority (PoA) consensus
    - Immediate finality
    - IBFT 2.0와 비교
      - incorporates a more optimized leader election process and reduces the number of communication rounds required to reach a consensus
  - IBFT 2.0
    - Byzantine Fault Tolerant (BFT) Proof of Authority (PoA) consensus
    - Immediate finality
  - Clique
    - Can operate with single validator
    - immediate finality X
    - Vulnerable to forks
...
- OTV (Outstanding Token Value)
  - cryptocurrency valuation metric that measures the total value of tokens that are planned and allocated for specific purposes, excluding idle or unallocated supply
  - **OTV = Token Price × Outstanding Supply**
  - OTV sits between Market Cap (current circulation) and FDV (maximum theoretical supply).



todo. incentiv. https://www.coingecko.com/learn/what-is-incentiv
