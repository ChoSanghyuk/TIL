...
- 트랜잭션 속성을 이용하는 포인트컷
  ...
  ![image-20230908083304928](img/image-20230908083304928-1696384640206-1.png)
...
### DI 프레임워크로서의 스프링 사용 방식 변화

- 어노테이션의 메타정보 활용
  - 자바 코드가 실행되는 것이 목적이 아닌 다른 자바 코드에 의해 데이터처럼 취급
  - 어플리케이션 핵심 로직을 담은 자바 코드, 이를 지원하는 IoC 방식의 프레임워크, 프레임워크가 참조하는 메타정보라는 3 가지 구성하는 방식에 잘 어울림
  - 리팩토링 시 IDE 활용하여 정보 수정 용이
    - XML은 패키지나 클래스 정보 등이 단순 텍스트로 작성되어 있어 리팩토링 시 번러롭고 위험함
    - 단, 어노테이션은 변경 시 매번 클래스를 새로 컴파일이 필요
- 정책과 관례를 이용한 프로그래밍
  - 코드 없이 미리 약속한 규칙/관계를 따라서 프로그램이 동작하도록 만드는 프로그래밍 스타일
  - 장점
    - 모든 작업을 과정을 직접 작성할 필요 X
  - 단점
    - 학습 비용
    - 잘못된 이해 시, 버그 찾기 힘들어짐
...
- `@Autowired`를 이용한 자동와이어링
  ...
  - 과정
    1. `@Autowired`가 붙은 경우, 파라미터 타입을 보고 주입 가능한 타입의 빈을 모두 찾음
    2. 주입 가능한 타입의 빈이 하나인 경우, 주입
    3. 두 개 이상인 경우, 프로퍼티와 동일한 이름의 빈이 있는지 찾아서 주입
    4. 최종 후보를 찾아내지 못하면 에러 발생

- `@ComponentScan`

  - 프로젝트 내의 모든 클래스패스를 다 뒤져서 `@ComponentScan` 어노테이션 클래스를 찾는 것은 부담이 가는 작업

    => 특정 패키지 아래에서만 찾도록 기준이 되는 패키지 지정
...
- 여러 개의 어노테이션에 공통적인 속성을 부여할 때 활용

- ex) 어노테이션 빈 스캔을 통해 자동등록 대상으로 인식되게하려면 어노테이션 정의에 `@Component`를 메타 어노테이션으로 붙여줌
  ...
  [리스트 7-106] @Component 메타 어노테이션을 가진 어노테이션 정의

  => `@SnsConnector` 부착 시 자동 빈 등록



### :bulb: 컨텍스트 분리와 @Import

- `@TestConfiguration`

  - 설명

    - 테스트에 대한 추가적인 빈이나 커스텀을 정의하거나 기존 빈을 override을 하는 데 사용할 수 있는 `@Configuration`
    - `@Configuration` 클래스와 달리 `@TestConfiguration`을 사용해도 `@SpringBootConfiguration`의 자동 감지를 막지 않음

  - 사용 방법

    1. 테스트 클래스에 `static inner class`를 만들어 빈을 @Autowire 하는 방법
    ...
    2. Test용 configuration 클래스를 만들고, 사용할 테스트에 `@Import`를 사용해 명시해주는 방법
...
- `@Import`
  ...
  - 설명
    - `@Import` 애너테이션을 사용하면 특정 컴포넌트를 다른 configuration으로 import 할 수 있음
    - import할 하나 이상의 컴포넌트 클래스(일반적으로 `@Configuration` 클래스)를 명시한다.
    - `Spring XML`의 `Import`와 같은 기능을 제공한다.
...
출처 : https://velog.io/@tjddus0302/Spring-TestConfiguration%EC%99%80-Import
...
- 개요
  ...
  - `@Profile` 
    - 실행환경에 따라 빈 구성이 달라지는 내용을 프로파일로 정의해서 만둘어두고, 실행 시점에 어떤 프로파일의 빈 설정을 사용할지 지정
    - 프로파일은 설정 클래스 단위로 지정
    - 프로파일이 지정되어 있지 않은 빈 설정은 default 프로파일로 취급되어, 항상 적용됨
    - `@Profile`이 붙은 설정 클래스는 `@Import`, `@ContextConfiguration`과 상관없이 현재 컨테이너의 활성 프로파일 목록에 자신의 프로파일 이름이 들어 있지 않으면 무시됨
...
- 중첩 클래스를 이용한 프로파일 적용
  ...
  - 스태틱 중첩 클래스를 이용하면 하나의 파일에서 여러 환경 관리 O
...
- 개요
  - 스프링 컨테이너는 이 클래스를 이용해 빈을 등록하고 관리
  - 컨테이너의 빈 등록 정보 확인
    - `getBeanDefinitionNames()` 메소드를 통해 컨테이너에 등록된 모든 빈 이름을 가져오고, 빈 이름을 이용해서 실제 빈과 빈 클래스 정보 조회
- 사용방법
  - `@Autowired`로 주입받아서 사용 O
...
:heavy_check_mark: **"자바 엔터프라이즈 개발을 편하게 해주는 오픈소스 경량급 애플리케이션 프레임워크"**

- 애플리케이션 프레임워크
  - 특정 계층이나, 기술, 업무 분야에 국한되지 않고 애플리케이션의 전 영역을 포괄하는 범용적인 프레임워크
  - 전 과정을 빠르고 편리하며 효율적으로 진행
- 경량급
  - 불필요하게 무겁지 않다
- 자바 엔터프라이즈 개발을 편하게
  - 앤터프라이즈 개발의 복잡함을 제거해내고 진정으로 개발을 편하게 해주는 해결책을 제시
  - 개발자가 실수하기 쉬운 로우레벨 기술에 많은 신경을 쓰지 않으면서도 애플리케이션의 핵심인 사용자의 요구사항 (비지니스 로직)을 빠르고 효과적으로 구현
- 오픈소스
  - 오픈소스 프로젝트 방식 but, 스프링소스 사업부가 개발 전담
...
- 엔터프라이즈 개발의 복잡함을 감소
  ...
  - 엔터프라이즈 개발의 복잡함
    ...
    - 늘어나는 기술적인 제약조건과 요구사항
      - 비지니스 로직 + 많은 사용자의 요청 처리 ~> 자원의 효율적 공유 및 분배까지 고려 필요
    - 비지니스 로직의 복잡함
    - 비지니스 로직의 복잡함과 기술적인 복잡함의 혼재

  - 해결

    - 비침투적인 방식을 통한 효과적인 해결책

      - 침투적 vs 비침투적

        - 침투적 : 어떤 기술을 적용했을 때 그 기술과 관련된 코드나 규약 등이 코드에 등장
        - 비침투적 : 기술의 적용 사실이 코드에 직접 반영되지 않음

      - 스프링 전략

        - 서비스 추상화

          - 기술에 대한 접근 방식에 일관성이 없고, 특정 환경에 종속적인 문제를 해결

        - AOP

          - 기술적인 처리를 담당하는 코드가 성격이 다른 코드에 섞여서 등장하는 문제 해결

          => 애플리케이션의 주요 기능과 비지니스 로직을 담은 코드만 독립적으로 존재

        :bulb: 비지니스 로직의 복잡함을 상대하는 전략은 결국 자바라는 객체지향 기술 그 자체.
...
- 조건
  ...
  1. 특정 규약에 종속되지 않는다
     - 꼭 필요한 API 외에는 종속되지 않아야 한다.
  2. 특정 환경에 종속되지 않는다
     - 비지니스 로직을 담고 있는 POJO 클래스는 웹 환경정보, 웹 기능을 담고 있는 클래스나 인터페이스를 사용해서는 X
     - 직접적으로 웹이라는 환경으로 제한해버리면 X
  3. 객체지향적으로 작성

- 장점

  - 특정한 기술과 환경에 종속되지 않는 오브젝트 => 깔끔한 코드
  - 객체지향적인 설계를 자유롭게 적용 O
...
- 스프링의 주요 기술인 **IoC/DI**, **AOP**와 **PSA**는 애플리케이션을 POJO로 개발할 수 있게 해주는 기능 기술
- IoC/DI
  - 장점 : "유연한 확장이 가능하게 하기 위함"
    - 개방 폐쇄 원칙 : 확장에 열려있고, 변경에 닫혀있는 구조
    - 폐쇄 관점에서의 장점으로는 **재사용이 가능함**
    - 예시
      - A -> B의 의존관계에서, B는 자유롭게 변경 가능(확장)하며, B가 변경되어도 A는 영향을 받지 않음(폐쇄)
      - B가 B1,B2,B3으로 변경되어도 A는 그대로 재사용이 가능
  - 활용
    - 핵심기능의 변경
      - 의존 대상의 변경
    - 핵심기능의 동적인 변경
      - 애플리케이션 동작 중간에 의존 대상 변경 O
    - 부가기능의 추가
      - 핵심기능은 그대로 둔 채로 부가기능을 추가
      - ex) 데코레이터 패턴
    - 인터페이스 변경
      - A가 C 오브젝트를 사용하지만, B 인터페이스를 구현하고, C는 B인터페이스를 구현하지 않는 경우
        - A -> B (C로 위임) -> C
        - B 인터페이스를 구현했으면서 내부에서 C를 호출해주는 어댑터 오브젝트를 A에 DI
    - 프록시
    - 템플릿과 콜백
    - 싱글톤과 오브젝트 스코프
      - 컨테이너가 알아서 싱글톤을 만들고 관리 => 클래스 자체는 싱글톤 고려 X
      - 다양한 스코프를 갖는 오브젝트 만들어 DI에 사용 O
    - 테스트
      - 효과적인 테스트 방법은 가능한 대상을 고립시키는 것
- AOP
- PSA(프터블 서비스 추상화)
...
- WAS
  ...
  - 다양한 형태의 모듈로 배포 가능
  - 고도의 안정성이나 고성능 시스템에서 필수적인 안정적인 리소스 관리, 레거시 시스템의 연동 O
- 경량급 WAS / 서블릿 컨테이너
  - 웹 모듈의 배포만 가능
  - 엔터프라이즈 애플리케이션 개발에 필요한 핵심 기능은 모두 이용  O
  - ex) 톰캣, 제티
...
- 계층형 아키텍쳐
  ...
  - 아키텍쳐 차원에서 책임과 성격이 다른 것을 크게 그룹으로 만들어 분리
  - 웹 기반 엔터프라이즈 애플리케이션은 일반적은 3계층 (3-tier / 3-layer) 애플리케이션
  ...
  - 계층의 경계를 넘어갈 때는 반드시 특정 계층에 종속되지 않는 오브젝트 형태로 변환
    ...
    - 잘못된 예시
      ...
      - `public ResultSet findUserByName(String name) throws SQLException;`
        ...
        - 데이터 엑섹스 계층의 기술과 그 역할을 다른 계층에 그대로 노출

        - 서비스 계층의 코드는 `ResultSet` 이라는 데이터 엑세스 계층에서 만들어지는 오브젝트를 직접 다뤄야함

          => 특정 데이터 엑세스 계층 기술에 종속 + 리소스 처리 등의 데이터 엑세스 작업 맡아야 함

        - JDBC 기술 종속적인 체크 예외 던질 경우, 서비스 계층에서 예외 해석 및 처리 코드 필요

          => 특정 데이터 엑세스 계층 기술에 종속 

    - 수정안

      - `public List<User> findUserByName(String name) throws DataAccessException;`
        - 특정 계층에 종속되지 않는 단순한 오브젝트 형태로 전달
          - `DataAccessException`와 같은 런타임 예외로 생성
            - 혹은 특정 구현 방식에 종속되지 않는 추상적인 형태로 생성

  - 계층 사이의 호출은 인터페이스를 통해 진행

    - 인터페이스 사용 = 각 계층 경계를 넘어서 들어오는 요청을 명확히 정의함
    - 단지 interface 키워드를 사용하는 것 X, 매우 신중하게 계층 내부의 예상되는 변화에도 쉽게 바뀌지 않도록 주의해서 설계

  - DI는 계층을 구분하지 X 때문에, 빈 사이의 의존관계를 만들 때 주의 필요

  - 주의점

    - 3 계층이라는 논리적이고 개념적인 구분. 꼭 오브젝트 단위로 만들어지는 것이 아님
      - 경우에 따라, 두 개의 계층 통합 O
...
- 엔터프라이즈 애플리케이션에 존재하는 정보를 단순히 데이터로 다루는 경우, 오브젝트로 다루는 경우로 구분

- 데이터 중심 아키텍쳐

  - 애플리케이션에 존재하는 정보를 단순히 값이나 값을 담기 위한 목적의 오브젝트 형태로 취급하는 구조

  - DB에서 돌려주는 내용을 그대로 맵 or 단순 결과 저장용 오브젝트에 넣어서 전달

  - DB/SQL 중심의 로직 구현 방식

    - 하나의 업무 트랜잭션에 모든 계층의 코드 종속되는 경향

    ![image-20231020174928605](토비의_스프링_1.assets/image-20231020174928605.png)

    [그림 9-17] DB 중심의 아키텍처

  - SQL에 변화가 있을 시, 로직들이 같이 변경

    - 데이터가 일종의 접착제 역할로 강한 결합을 생성
    - 데이터 전달이 `List`, `Map`과 같은 타입으로 진행 => SQL이 변경되어 필드의 수, 개수, 이름이 변경될 경우 서비스 계층과 프레젠테이션 계층의 코드들도 같이 변경 필요

  - 변화에 매우 취약

    - 객체지향의 장점 활용 X
    - 업무 트랜잭션에 따라 필드 하나가 달라도 거의 비슷한 DAO 메소드를 새로 생성 필요

  - 제한된 DB에 의존

    - 복잡한 SQL로 로직을 처리

    - 복잡한 SQL을 처리하기 위해 제한된 자원인 DB에 큰 부담을 주는 것은 바람직하지 X

      <=> 어플리케이션 서버는 확장이 용이

    => 로직을 서버로 가져오는 것이, 비용, 안정성, 코드 검증 측면에서 유리

  - 거대한 서비스 계층 방식

    - 주요 로직을 서비스 계층의 코드에서 처리
    - DAO는 단순한 결과를 반환. 서비스 계층에서 이를 분석, 가공하면서 비지니스 로직에 적용

    ![image-20231020174852435](토비의_스프링_1.assets/image-20231020174852435.png)
  ...
  - 단점
    ...
    - 여전히 큰 계층 간 결합도
      - 데이터 엑세스 계층의 SQL은 서비스 계층의 비지니스 로직의 필요에 따라 만들어짐
    - 코드간 중복 존재
      - 서비스 계층의 메소드는 크기가 큰 업무 트랜잭션 단위로 만들어지기 때문에 비슷한 기능의 코드가 여러 메소드에서 중복됨
      - 포맷과 타입이 DAO 메소드마다 다르기 때문에 공통적인 처리 기능 사용 어려움

- 오브젝트 중심 아키텍처

  - 도메인 모델을 반영하는 오브젝트 구조를 만들어주고, 그것을 각 계층 사이에서 정보를 전송하는 데 사용

    - 객체지향 분석과 모델링의 결과로 나오는 도메인 모델을 오브젝트 모델로 활용

    - 관계형 DB  엔터티 구조와 유사해짐

    - 도메인 모델이 반영된 도메인 오브젝트도 전 계층에서 일관된 구조를 유지 O

    - 레퍼런스 변수를 이용해서 타 오브젝트와의 관계 표현 O
  ...
  - 장점
    ...
    - 계층간 낮은 결합도
      - 어떤 DAO를 이용해서 오브젝트를 가져왔는지 중요하지 X
    - 직관적인 메소드
      - `calcTotalOfProductPrice()`등 메소드 이름을 통해 작업 명시 O
    - 재사용 가능한 코드
      - 독립적인 오브젝트이기에, 계층 내 어디서든 사용 O
    - 레퍼런스를 통한, 오브젝트 구조 파악 용이 

  - 문제점

    - 성능 문제
      - 최적화된 SQL을 매번 만들어 사용하는 것보단 성능 면에서 떨어질 수 있음
    - 메모리 낭비
      - 특정 비지니스 로직에서 필요한 정보가 몇 개의 필드뿐이라면, DAO에서 도메인 오브젝트의 모든 필드 정보를 채워서 전달하는 것은 낭비
      - Product 하나만 필요한 경우에도 관계를 가진 Category 오브젝트까지 조회

  - 해결

    - Lazy Loading
      - 최소한의 오브젝트 정보만 읽어두고 관계하고 있는 오브젝트가 필요한 경우에만 다이내맥하게 읽어옴
      - JPA, JDO, 하이버네이트 등 ORM 기술 활용
      - 코드 테이블처럼 자주 참조되는 것은 오브젝트 캐시로 생성

  - 종류

    - 빈약한 도메인 오브젝트 방식

      ![image-20231020174816515](토비의_스프링_1.assets/image-20231020174816515.png)

      - 정보만 담겨 있고, 정보를 활용하는 아무런 기능도 갖고 있지 않음
      - 핵심 비지니스 로직은 서비스 계층에 모두 존재
      - 로직의 재사용성 :arrow_down:, 중복 발생
      - 비지니스 로직이 복잡하지 않다면 만들기 쉽고, 3 계층의 구조의 특징을 잘 살려서 개발 O

    - 풍성한 도메인 오브젝트 방식 (영리한 도메인 오브젝트 방식)

      ![image-20231020174618262](토비의_스프링_1.assets/image-20231020174618262.png)

      - 도메인 오브젝트의 객체지향적인 특징을 잘 살림
      - 특정 도메인 오브젝트와 관계가 깊은 비지니스 로직을 도메인 오브젝트에 넣고, 서비스 계층에서 재사용
      - 높은 응집도
        - 데이터와 그것을 사용하는 기능이 한곳에 모여있음

    - 도메인 계층 방식

      - AspectJ를 활용해서 도메인 오브젝트에 타 계층의 빈을 등록

      - 복잡하고 잦은 변경이 필요한 경우에 주의 가지고 사용
