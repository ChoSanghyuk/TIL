...
- JDBC 한계
  ...
  - 호환성 없는 SQLException의 DB 에러 정보
    ...
    - SQLException의 `getSQLSate()`
      - Open Group의 XOPEN SQL 스페엑 정의 SQL 상태 코드를 따름
...
- 스프링의 **트랜잭션 동기화**
  ...
  ```
  TransactionSynchronizationManager and @Transactional are both components used in Spring Framework for managing transactions, but they serve different purposes and operate at different levels within the application. Let's compare them:
  
  TransactionSynchronizationManager:
  
  TransactionSynchronizationManager is a class provided by Spring Framework that acts as a central registry for transaction synchronization callbacks.
  It allows different parts of the application to register synchronization callbacks that will be executed at various stages of the transaction's lifecycle, such as beforeCommit, afterCommit, afterCompletion, etc.
  This low-level API provides more control over transaction synchronization and allows customizing behavior during different phases of the transaction.
  @Transactional:
  
  @Transactional is a declarative annotation used to mark methods or classes as transactional in Spring applications.
  It is a high-level abstraction that simplifies transaction management by providing a declarative way to define the transaction boundaries.
  When a method or class is marked with @Transactional, Spring automatically manages the transaction lifecycle, starting a new transaction before the method is executed and committing or rolling back the transaction based on the method's outcome.
  The @Transactional annotation allows developers to focus on business logic without worrying about the low-level details of transaction management.
  In summary, the key differences are:
  
  Level of Abstraction:
  
  TransactionSynchronizationManager is a low-level API for managing transaction synchronization callbacks, providing more control and customization options.
  @Transactional is a high-level, declarative way to manage transactions, abstracting away the low-level details and allowing developers to work with transactions at a higher level.
  Usage:
  
  TransactionSynchronizationManager is typically used when you need fine-grained control over transaction synchronization and require custom logic at different phases of the transaction.
  @Transactional is commonly used for most transactional scenarios, where you want Spring to manage the transactions automatically, without the need to explicitly deal with transaction handling in your business logic.
  In practice, most developers use @Transactional for standard transaction management needs. Only in rare cases where custom transaction synchronization behavior is required, developers may use TransactionSynchronizationManager directly.
  ```
