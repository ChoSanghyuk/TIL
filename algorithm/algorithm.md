...
`[e1, e2, ..., en]`에 대해서 `p ~ q`까지의 숫자 중 `abs(ej - x)` 값이 최소가 되는 x 값을 구하여라

=> p ~ q까지의 수에 대해서 하나씩 절대 차이값을 구하는 것이 아닌, 최소값이 될 수 있는 후보들을 먼저 골라냄

=> 후보 : p, q, 임의의 i에 대하여 p보다 크고, q보다 작은 `( e_i + e_(i+1) )/2` 값

예시 문제 : [Sherlock and MiniMax](https://www.hackerrank.com/challenges/sherlock-and-minimax/problem)
